/**
 * IL-42 Selective Prerender Script — /adams-co-homes-for-sale
...
async function prerenderAdamsCo() {
  console.log("[prerender-adams-co] Starting SSR build for /adams-co-homes-for-sale...");
  await build({
    root: resolve(ROOT, "client"),
    configFile: resolve(ROOT, "vite.config.ts"),
    build: {
      ssr: resolve(ROOT, "client/src/entry-server-adams-co.tsx"),
      outDir: resolve(ROOT, "dist/server"),
      emptyOutDir: false,
      rollupOptions: {
        output: {
          format: "esm",
          entryFileNames: "entry-server-adams-co.js",
        },
      },
    },
    css: {},
  ...
  if (!existsSync(shellPath)) {
    throw new Error(`[prerender-adams-co] dist/public/index.html not found. Run 'vite build' first.`);
  }
  ...
  const srcOutputPath = resolve(srcOutputDir, "adams-co-homes-for-sale.html");
  writeFileSync(srcOutputPath, prerenderedShell, "utf-8");
  console.log(`[prerender-adams-co] Committed artifact written to: ${srcOutputPath}`);
  ...
  console.log(`[prerender-adams-co] Sanity: rootNotEmpty=${rootNotEmpty}, hasH1=${hasH1}, anchors=${anchorCount}`);
  if (!rootNotEmpty) throw new Error("[prerender-adams-co] FAIL: Root div still empty.");
  console.log("[prerender-adams-co] Done.");
}
prerenderAdamsCo().catch((err) => {
  console.error("[prerender-adams-co] FAILED:", err.message);
  process.exit(1);
});
