/**
 * IL-35 Selective Prerender Script — /old-north-end-colorado-springs-real-estate
...
async function prerenderOldNorthEnd() {
  console.log("[prerender-old-north-end] Starting SSR build for /old-north-end-colorado-springs-real-estate...");
  await build({
    root: resolve(ROOT, "client"),
    configFile: resolve(ROOT, "vite.config.ts"),
    build: {
      ssr: resolve(ROOT, "client/src/entry-server-old-north-end.tsx"),
      outDir: resolve(ROOT, "dist/server"),
      emptyOutDir: false,
      rollupOptions: {
        output: {
          format: "esm",
          entryFileNames: "entry-server-old-north-end.js",
        },
      },
    },
    css: {},
  });
  console.log("[prerender-old-north-end] SSR build complete. Rendering HTML...");
  const ssrModule = await import(resolve(ROOT, "dist/server/entry-server-old-north-end.js"));
  const html = ssrModule.renderOldNorthEndColoradoSpringsRealEstate();
  console.log(`[prerender-old-north-end] Rendered HTML length: ${html.length} chars`);
  const shellPath = resolve(ROOT, "dist/public/index.html");
  if (!existsSync(shellPath)) {
    throw new Error(`[prerender-old-north-end] dist/public/index.html not found. Run 'vite build' first.`);
  }
  const shell = readFileSync(shellPath, "utf-8");
  const PLACEHOLDER = '<div id="root"></div>';
  if (!shell.includes(PLACEHOLDER)) {
    throw new Error(`[prerender-old-north-end] Could not find '${PLACEHOLDER}' in dist/public/index.html.`);
  }
  ...
  console.log(`[prerender-old-north-end] Committed artifact written to: ${srcOutputPath}`);
  ...
  console.log(`[prerender-old-north-end] Sanity: rootNotEmpty=${rootNotEmpty}, hasH1=${hasH1}, anchors=${anchorCount}`);
  if (!rootNotEmpty) throw new Error("[prerender-old-north-end] FAIL: Root div still empty.");
  console.log("[prerender-old-north-end] Done.");
}
prerenderOldNorthEnd().catch((err) => {
  console.error("[prerender-old-north-end] FAILED:", err.message);
  process.exit(1);
});
